...
return new class extends Migration
{
    public function up(): void
    {
        ...
        match ($driver) {
            ...
            'mysql', 'mariadb' => $this->updateMySqlEnum(['PRS', 'RRSP', 'IIRUP', 'IIRUSP', 'RRSEP']),
            default => null,
        };
    }
    ...
    {
        DB::table('disposals')
            ->where('document_type', 'PRS')
            ->update(['document_type' => 'IIRUP']);

        DB::table('disposals')
            ->where('document_type', 'RRSP')
            ->update(['document_type' => 'RRSEP']);

        $driver = Schema::getConnection()->getDriverName();

        match ($driver) {
            'sqlite' => $this->updateSqliteTable(includeNewTypes: false),
    ...
    private function updateMySqlEnum(array $values): void
    {
        ...
        DB::statement(
            "ALTER TABLE disposals MODIFY document_type ENUM('{$enumList}') NOT NULL DEFAULT 'PRS'"
        );
    }

    private function updateSqliteTable(bool $includeNewTypes = true): void
    {
        $tempTable = 'disposals_tmp_prs_rrsp';
        $documentTypes = $includeNewTypes
            ? ['PRS', 'RRSP', 'IIRUP', 'IIRUSP', 'RRSEP']
            : ['IIRUP', 'IIRUSP', 'RRSEP'];
        $documentTypeDefault = $includeNewTypes ? 'PRS' : 'IIRUP';

        Schema::disableForeignKeyConstraints();

        Schema::create($tempTable, function (Blueprint $table) use ($documentTypes, $documentTypeDefault): void {
            $table->id();
        ...
        Schema::rename($tempTable, 'disposals');
