...
@php
    ...
    $condition = $disposal->item_disposal_condition
        ?? (! in_array($disposal->disposal_type, ['sale', 'destruction'], true) ? 'others' : 'unserviceable');
    $conditionOther = $disposal->item_disposal_condition_other
        ?? ($condition === 'others' ? ($disposal->disposal_type_other ?? '') : '');
    ...
    $methodOther = $disposal->disposal_method_other
        ?? ($method === 'others' ? ($disposal->disposal_type_other ?? '') : '');
@endphp
...
<table style="margin-bottom:0;">
    <tr>
        <td colspan="6" style="font-size:9pt; padding:4px 6px;">
            ...
            <span class="chk">{{ $condition === 'unserviceable' ? '☑' : '☐' }}</span> Unserviceable&nbsp;&nbsp;&nbsp;
            <span class="chk">{{ $condition === 'no_longer_needed' ? '☑' : '☐' }}</span> No longer needed&nbsp;&nbsp;&nbsp;
            <span class="chk">{{ $condition === 'obsolete' ? '☑' : '☐' }}</span> Obsolete&nbsp;&nbsp;&nbsp;
            <span class="chk">{{ $condition === 'others' ? '☑' : '☐' }}</span> Others ({{ $conditionOther }})
        </td>
    </tr>
</table>
...
<table>
    ...
    <tbody>
        ...
        <tr class="item-rows">
            ...
            <td class="numeric">{{ number_format($line->appraised_value ?? $disposal->appraised_value ?? $line->carrying_amount ?? 0, 2) }}</td>
        ...
        @endfor
        <tr>
            <td colspan="8" class="center" style="font-style:italic;">------nothing follows-----</td>
        </tr>
    </tbody>
</table>
...
<table style="margin-bottom:0;">
    ...
    <tr>
        <td colspan="2" style="padding:4px 10px; font-size:9pt;">
            ...
            <span class="chk">{{ $method === 'public_auction' || ($disposal->sale_amount ?? 0) > 0 ? '☑' : '☐' }}</span> Public Auction&nbsp;&nbsp;&nbsp;
            <span class="chk">{{ $method === 'destruction' ? '☑' : '☐' }}</span> Destruction&nbsp;&nbsp;&nbsp;
            <span class="chk">{{ $method === 'throwing' ? '☑' : '☐' }}</span> Throwing&nbsp;&nbsp;&nbsp;
            <span class="chk">{{ $method === 'others' ? '☑' : '☐' }}</span> Others ({{ $methodOther }})
        </td>
    </tr>
</table>
