...
<div class="min-h-screen bg-gray-100">
    ...
    <div class="bg-[#1a2c5b] border-b-4 border-[#c8a84b] shadow-lg">
        <div class="w-full px-4 sm:px-6 lg:px-8 py-4">
            ...
            <p class="text-white font-bold text-lg leading-tight mt-0.5">Edit Item</p>
    ...
    <div class="w-full px-4 py-6 sm:px-6 lg:px-8">
        <form method="POST" action="{{ route('items.update', $item) }}" class="space-y-5" x-data="itemForm()">
            @csrf @method('PUT')
            ...
            <div class="bg-white border border-gray-200 rounded shadow-sm overflow-hidden">
                ...
                <div class="p-5 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    ...
                    <div class="flex flex-col gap-1.5 md:col-span-2 lg:col-span-3">
                        ...
                        <textarea name="description" rows="2"
                            ...
                            placeholder="Optional detailed description">{{ old('description', $item->description) }}</textarea>
                    </div>
                    <div class="flex flex-col gap-1.5">
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <input name="unit_cost" type="number" step="0.01" min="0.01" value="{{ old('unit_cost', $item->unit_cost) }}" x-model.number="unitCost"
                            class="rounded border border-gray-300 bg-gray-50 px-3 py-2 text-sm focus:border-[#1a2c5b] focus:bg-white focus:outline-none focus:ring-1 focus:ring-[#1a2c5b]" required>
                    </div>
                    <div class="flex flex-col gap-1.5">
                        <label class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Est. Useful Life</label>
                        <input name="estimated_useful_life" value="{{ old('estimated_useful_life', $item->estimated_useful_life) }}"
                            class="rounded border border-gray-300 bg-gray-50 px-3 py-2 text-sm focus:border-[#1a2c5b] focus:bg-white focus:outline-none focus:ring-1 focus:ring-[#1a2c5b]"
                ...
                <div class="px-5 pb-5 border-t border-gray-100 pt-4">
                    <label class="inline-flex items-center gap-3 cursor-pointer">
                        ...
                        <span class="text-xs text-gray-400">(Inactive items won&rsquo;t appear in search autocomplete)</span>
                    </label>
                </div>
            </div>
...
function itemForm() {
    return {
        unitCost: {{ old('unit_cost', $item->unit_cost) }},
        get classInfo() {
