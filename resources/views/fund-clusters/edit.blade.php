...
<div class="min-h-screen bg-gray-100">
    ...
    <div class="bg-[#1a2c5b] border-b-4 border-[#c8a84b] shadow-lg">
        <div class="w-full px-4 sm:px-6 lg:px-8 py-4">
            ...
            <p class="text-white font-bold text-lg leading-tight mt-0.5">Edit Fund Cluster</p>
        </div>
    </div>
    ...
    <div class="w-full px-4 py-6 sm:px-6 lg:px-8 max-w-2xl">
        <div class="bg-white border border-gray-200 rounded shadow-sm overflow-hidden">
            ...
            <form method="POST" action="{{ route('fund-clusters.update', $fundCluster) }}" class="p-5 space-y-4">
                @csrf @method('PUT')

                <div>
                    <label class="block text-xs font-bold uppercase tracking-widest text-gray-600 mb-1">Code</label>
                    <input type="text" name="code" value="{{ old('code', $fundCluster->code) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $fundCluster->name) }}" required
                           class="w-full rounded border border-gray-300 text-sm px-3 py-2">
                    @error('name') <p class="text-xs text-rose-600 mt-1">{{ $message }}</p> @enderror
                </div>

                <div class="flex items-center gap-3 pt-2">
                    <button type="submit" class="rounded bg-[#1a2c5b] px-6 py-2 text-sm font-semibold text-white hover:bg-[#253d82] transition">Update Fund Cluster</button>
